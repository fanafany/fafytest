...
def url_open(url):
    headers = {
        ...
        'Referer':'https://www.mzitu.com/186217/3'
...
#获取当前图片组的最大页码数
def get_maximage(url):
    html = url_open(url).decode('utf-8')
    page = re.findall(r'<span>\d{1,2}',html)
    return page[-1][6:len(page[-1])]
...
def download(folder = 'meizi',*pages):
    ...
    for page in pages:
        ...
        os.chdir(pagefolder)
        #获取图片组地址列表
        img_group_addr = find_imgs(page_url)
        #对于每个图片组，获取图片地址并保存
        group = 0
        for addr in img_group_addr:
            group += 1
            img_addrs = [find_img(addr,x)for x in range(int(get_maximage(addr)))]
            #创建文件夹
